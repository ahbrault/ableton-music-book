...
async function processHtmlFile(filePath, fileName) {
    ...
    console.log(`[INFO] Found ${imagesToProcess.length} image(s) to optimize in ${fileName}.`);
    ...
    for (const image of imagesToProcess) {
        try {
            ...
            console.log(`  [OK] Image ${image.index + 1} optimized and saved to ${publicPath}`);

        } catch (error) {
            console.error(`  [ERROR] Failed to process image ${image.index + 1} in ${fileName}:`, error);
        }
    }
...
async function runOptimizer() {
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('Please make sure the path in the CONFIG object is correct.');
        }
    }
